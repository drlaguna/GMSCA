...
createGCN <- function( expr.data,
  ...
  beta = min(b.study$fitIndices[as.numeric(b.study$fitIndices$SFT.R.sq) > r.sq.cutoff & as.numeric(b.study$fitIndices$slope) < 0 & as.numeric(b.study$fitIndices$max.k) > max.k.cutoff,"Power"])
...
#' Secondary Coexpression Network Creation
#'
#' This function creates a Secondary Coexpression Network (SCN)
#' @param net.filename The full path of a file containing a GCN created with \code{\link{createGCN}}
#' @param expr.data can be a full path file name or a data frame with genes in columns and samples in
#' rows, giving the expression data used to construct the GCN network.
#' @param target.celltype A string with the name of the celltype we are interested in. This name should be
...
#' @return a list with two objects: The GCN network obtained using the expression data filtered to show the
#' secondary role of genes, and a matrix with that expression data.
#' @export
createSCN = function( net.filename, expr.data, target.celltype, markers.path = NULL ) {
  ...
  # Tabla de enriquecimiento celular
  enrichment.filename = paste0( net.filename, ".celltype.csv")
  if ( file.exists( enrichment.filename ) ) {
    cell.type.by.module = read.csv( enrichment.filename, stringsAsFactors=F)
  } else {
    if ( is.null( markers.path ) ) markers.path = dirname( net.filename )
    cell.type.by.module = getModulesEnrichment( net = net, markers.path = markers.path)
    write.csv( cell.type.by.module, enrichment.filename )
  }
  # Nos quedamos por un lado con los nombres de las columnas ( tipos de celula )
  cell.type.names = cell.type.by.module[,1]
  ...
  cell.type.by.module = cell.type.by.module[,-1][,as.vector(apply(cell.type.by.module[,-1], 2, FUN = function(x) { sum( x <= 0.05)} ) == 1)]
  # Devolvemos la lista de módulos etiquetada por el tipo de célula en el que están significativa y exclusivamente enriquecidos
  enriched.modules = apply( cell.type.by.module, 2, FUN = function(x) { cell.type.names[which(x <= 0.05)] } )
  modules.to.clean = enriched.modules[ enriched.modules != target.celltype]
  cleaned.data = expr.data[,!(net$moduleColors %in% names(modules.to.clean))]

  for ( module in names(modules.to.clean) ) {
    # Nos quedamos con el eigengene del módulo
    ee = net$MEs[paste0("ME", module)]
    # Nos quedamos con la expresión correspondiente a los genes del módulo que nos interesa
    expr = expr.data[,net$moduleColors == module]
    print(paste0("Corrigiendo modulo ", module) )
    # Aplicamos el modelo lineal para quedarnos con los residuos
    res = apply( expr, 2, function(y) { lm( y ~ ., data=ee )$residuals })
    # Creamos la nueva expresión sustituyendo el módulo orignal por los residuos
    cleaned.data = cbind(cleaned.data, res)
  }
  net$type = "signed"
  if ( !is.null(net$type)) net.type = net$type
  cleaned.net = createGCN( expr.data = cleaned.data, net.type = net.type, save.adj.filename=paste0( net.filename, ".Just.", target.celltype, ".adjs.rds"), save.tom.filename=paste0( net.filename, ".Just.", target.celltype, ".tom.rds") )
